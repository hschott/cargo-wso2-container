...
public abstract class AbstractWSO2Carbon4xAdminService extends LoggedObject
	implements WSO2BaseAdminService {
    ...
    public AbstractWSO2Carbon4xAdminService(URL url, String wso2username,
	...
	try {
	    AuthenticationAdminStub authenticationStub = new AuthenticationAdminStub(
		    new URL(getUrl() + "/services/AuthenticationAdmin")
			    .toString());
	    prepareServiceClient(authenticationStub._getServiceClient());
	    if (authenticationStub.login(wso2username, wso2password,
		    url.getHost())) {
		...
		getLogger().info(
			"Authentication to " + getUrl() + " successful.",
			getClass().getSimpleName());
		return;
	    }
	} catch (Exception e) {
	    throw new WSO2AdminServicesException(
		    "exception authenticating user " + wso2username, e);
	}
	...
	Protocol.registerProtocol("https", new Protocol("https",
		easySSLProtocolSocketFactory, getUrl().getPort() == -1 ? 443
			: getUrl().getPort()));
    }
	...
	getLogger().debug(
		"Check deploy status of [" + deployable.getFile() + "] on "
			+ getUrl(), getClass().getSimpleName());
    }

    void logRemove(Deployable deployable) {
	getLogger().info(
		"Remove [" + deployable.getFile() + "] from " + getUrl(),
		getClass().getSimpleName());
    }

    void logStart(Deployable deployable) {
	getLogger().info(
		"Starting [" + deployable.getFile() + "] on " + getUrl(),
		getClass().getSimpleName());
    }

    void logStop(Deployable deployable) {
	getLogger().info(
		"Stopping [" + deployable.getFile() + "] on " + getUrl(),
		getClass().getSimpleName());
    }

    void logUpload(Deployable deployable) {
	getLogger().info(
		"Uploading [" + deployable.getFile() + "] to " + getUrl(),
		getClass().getSimpleName());
	...
	serviceClient.getOptions().setProperty(HTTPConstants.COOKIE_STRING,
		sessionCookie);
	...
	if (httpUsername != null && httpUsername.length() >= 0
	    ...
	    serviceClient.getOptions().setProperty(HTTPConstants.AUTHENTICATE,
		    authenticator);
	}
    }
