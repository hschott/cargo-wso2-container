...
public class WSO2DeployableTest {
	...
	deployable = (WSO2WAR) DEPLOYABLE_FACTORY.createDeployable(
	...
	Assert.assertEquals("berlin#4.1-SNAPSHOT.war",
	...
	file.mkdir();
	deployable = (WSO2WAR) DEPLOYABLE_FACTORY.createDeployable(
		WSO2Carbon4xContainer.ID, file.getPath(), DeployableType.WAR);
	...
	deployable.setVersion("4.1-SNAPSHOT");
	Assert.assertEquals("berlin#4.1-SNAPSHOT",
		deployable.getApplicationName());
    }
	...
	deployable = (CarbonApplication) DEPLOYABLE_FACTORY.createDeployable(
		WSO2Carbon4xContainer.ID, "test.car", CarbonApplication.TYPE);

	Assert.assertEquals("test", deployable.getApplicationName());
    }
	...
	deployable = (Axis2Module) DEPLOYABLE_FACTORY.createDeployable(
		WSO2Carbon4xContainer.ID, mar.getFile(), Axis2Module.TYPE);
	...
	deployable = (Axis2Service) DEPLOYABLE_FACTORY.createDeployable(
	...
	deployable = (WSO2Axis2Service) DEPLOYABLE_FACTORY.createDeployable(
	...
	deployable = (BAMToolbox) DEPLOYABLE_FACTORY.createDeployable(
	...
	deployable = (WSO2Connector) DEPLOYABLE_FACTORY.createDeployable(
		WSO2Carbon4xContainer.ID, zip.getFile(), WSO2Connector.TYPE);

	Assert.assertEquals("{org.wso2.carbon.connectors}twitter", deployable.getApplicationName());
    }
