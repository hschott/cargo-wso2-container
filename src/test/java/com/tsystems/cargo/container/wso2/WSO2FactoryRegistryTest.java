...
public class WSO2FactoryRegistryTest
{
    ...
    public void testRemoteConfigurationInstance()
    {
        ...
        Assert.assertThat(configuration, CoreMatchers.instanceOf(WSO2RuntimeConfiguration.class));

        Container container =
            CONTAINER_FACTORY.createContainer(WSO2Carbon4xContainer.ID, ContainerType.REMOTE,
                configuration);

        Assert.assertThat(container, CoreMatchers.instanceOf(WSO2Carbon4xRemoteContainer.class));

        Deployer deployer = DEPLOYER_FACTORY.createDeployer(container);

        Assert.assertThat(deployer, CoreMatchers.instanceOf(WSO2Carbon4xRemoteDeployer.class));
    }
    ...
    public void testExistingConfigurationInstance()
    {
        ...
        Assert.assertThat(container,
            CoreMatchers.instanceOf(WSO2Carbon4xInstalledLocalContainer.class));

        Deployer deployer = DEPLOYER_FACTORY.createDeployer(container);

        Assert.assertThat(deployer,
            CoreMatchers.instanceOf(WSO2Carbon4xInstalledLocalDeployer.class));
    }
