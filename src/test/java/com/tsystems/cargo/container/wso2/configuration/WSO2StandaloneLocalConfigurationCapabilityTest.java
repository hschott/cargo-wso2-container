...
public class WSO2StandaloneLocalConfigurationCapabilityTest {
	...
	Assert.assertTrue(config
		.supportsProperty(DatasourcePropertySet.CONNECTION_TYPE));
	Assert.assertTrue(config.supportsProperty(GeneralPropertySet.RMI_PORT));
	Assert.assertTrue(config
		.supportsProperty(WSO2CarbonPropertySet.CARBON_USERNAME));
	Assert.assertTrue(config
		.supportsProperty(WSO2CarbonPropertySet.CARBON_PASSWORD));
	Assert.assertTrue(config
		.supportsProperty(WSO2CarbonPropertySet.CARBON_CONTEXT_ROOT));
	Assert.assertTrue(config
		.supportsProperty(WSO2CarbonPropertySet.CARBON_SERVER_ROLES));
